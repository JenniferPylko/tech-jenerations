...
manifest.versionId = manifest.versionId.replace(/[^.]*$/, parseInt(process.env.GITHUB_RUN_NUMBER ?? 0) - parseInt(process.env.BASE_RUN_NUMBER ?? 0))
...
await writeFile("config/bcc-common.toml", bcc.replace(/modpackVersion = ".*\..*\..*"/g, `modpackVersion = "${manifest.versionId}"`))
